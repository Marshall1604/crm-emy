...
interface EditInsuranceModalProps {
  client: InsuranceClient | null;
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (updatedClient: InsuranceClient) => void;
...
export function EditInsuranceModal({
  ...
  useEffect(() => {
    if (client) {
      ...
      setDob(client.dob || '');
      setState(client.state || 'CA');
      setZipCode(client.zipCode || '');
      setStatus(client.status || 'Mới');
      setAssignedStaff(client.assignedStaff || 'Amy Tran');
      if (client.carrier && defaultCarriers.includes(client.carrier)) {
        setCarrier(client.carrier);
        setCustomCarrier('');
      } else if (client.carrier) {
        setCarrier('Khác');
        setCustomCarrier(client.carrier);
      } else {
        setCarrier('UnitedHealthcare');
        setCustomCarrier('');
      }
      setInsuranceType(client.insuranceType || 'Medicare Advantage');
      setPlanName(client.planName || '');
      setEffectiveDate(client.effectiveDate || '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedRecord: InsuranceClient = {
      ...client,
  ...
  const handleDelete = () => {
    ...
    const confirmMsg =
      language === 'vi'
        ? `Bạn có chắc chắn muốn xóa hồ sơ bảo hiểm của "${client.fullName}" không?`
        : `Are you sure you want to delete the insurance record for "${client.fullName}"?`;
      ...
      <DialogContent className="business-modal">
        ...
        <header className="modal-header">
          ...
          <div>
            <DialogTitle>
              {language === 'vi'
                ? `Hồ Sơ Khách Hàng: ${client.fullName}`
                : `Insurance Profile: ${client.fullName}`}
            </DialogTitle>
            <DialogDescription>
              {language === 'vi'
                ? `ID: ${client.id} · Cập nhật gần nhất: ${new Date(client.updatedAt).toLocaleDateString('vi-VN')}`
                : `ID: ${client.id} · Last updated: ${new Date(client.updatedAt).toLocaleDateString('en-US')}`}
            </DialogDescription>
          </div>
        </header>
          ...
          <div className="modal-scroll">
            ...
            >
              <div className="modal-grid cols-2">
                ...
                <FormField
                  label={language === 'vi' ? 'Tên gói bảo hiểm' : 'Plan Name'}
                  span="span-2"
                >
                  <Input
                    placeholder={
                      language === 'vi'
                        ? 'Ví dụ: Aetna Medicare Value Plus Plan (HMO)'
                        : 'e.g. Aetna Medicare Value Plus Plan (HMO)'
                    }
                    value={planName}
              ...
              <div className="modal-grid cols-1">
                <FormField label={language === 'vi' ? 'Nội dung ghi chú' : 'Case Notes'}>
                  <Textarea
                    ...
                    placeholder={
                      language === 'vi'
                        ? 'Ví dụ: Muốn tư vấn lại vào tháng 10 đợt AEP...'
                        : 'e.g. Follow up in October during AEP...'
                    }
                    value={notes}
